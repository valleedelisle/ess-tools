...
class Mariapod(object):
  ...
  def create_pvc(self, access_mode='ReadWriteMany', storage='50Gi', volume_suffix='data'):
    ...
    requirements = self.client.V1ResourceRequirements(requests={'storage': storage})
  ...
  def app_deployment(self, deployment):
    ...
    self.resp = self.apps_api.create_namespaced_deployment(
        body=deployment,
        namespace=self.namespace)
    print('App Deployment created. status="%s"' % str(self.resp))
...
def main():
  args = parse_args()
  db_object = Mariapod(**args.__dict__)
