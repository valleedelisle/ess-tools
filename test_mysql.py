...
class Shiftdeployment(object):
  ...
  def get_metadata(self, object_name):
    return self.client.V1ObjectMeta(name=object_name + "-" + self.name, labels={"app": self.name})

  def get_selector(self):
    return {'matchLabels': {'app': self.name}}
  ...
  def create_pod(self):
    ...
    return self.client.V1Deployment(
        api_version="apps/v1",
        kind="Deployment",
        metadata=self.get_metadata("deploy"),
        spec=spec
    )
  ...
  def post_pvc(self, deployment):
    # Create deployement
    print("Core Deployment %s" % deployment)
    self.resp = self.core_api.create_namespaced_persistent_volume_claim(
        body=deployment,
        namespace=self.namespace)
    print("Core Deployment created. status='%s'" % str(self.resp.status))
...
def main():
  ...
  #debug=args.debug, token=args.token, name=args.name, namespace=args.namespace)
